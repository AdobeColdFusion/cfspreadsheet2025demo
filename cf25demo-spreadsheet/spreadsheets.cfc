component spreadsheet{
    ...
    public function displayDataFiles(){
        ...
        for (item in filteredResults){ // Here we build the links which will allow you to view the data from each CSV.
            dataItem &= "<li>";
            dataItem &= "<form>";
            dataItem &= "<input type='hidden' name='filetoview' value='#request.datadiskpath#/#item.name#' />";
            dataItem &= "<button class='button is-ghost' hx-post='displaydata.cfm' hx-target='##viewcontainer'>";
            dataItem &= item.name;
            dataItem &= "</button>";
            dataItem &= "</form>";
            dataItem &= "</li>";
    ...
    public function displayDataFilesForEdit(){
        ...
        for (item in filteredResults){ // Here we build the links which will allow you to view the data from each CSV.
            ...
            dataItem &= "<form>";
            dataItem &= "<input type='hidden' name='filetoview' value='#request.datadiskpath#/#item.name#' />";
            dataItem &= "<button class='button is-ghost' hx-post='editdata.cfm' hx-target='##viewcontainer'>";
            dataItem &= item.name;
            dataItem &= "</button>";
            dataItem &= "</form>";
            dataItem &= "</li>";
    ...
    public function displayDataFilesForSearch(){
        ...
        dataItem &= "<li>";
        dataItem &= "<form>";
        dataItem &= "<select name='filename' id='filename' class='input'>";
        for (item in filteredResults){ // Here we build the links which will allow you to view the data from each CSV.
            dataItem &= "<option value='#item.name#'>" & item.name & "</option>";
            dataItem &= item.name;
            dataItem &= "";
        }
        dataItem &= "</select><div class='block'></div>";
        dataItem &= "<input type='text' name='searchparam' class='input' id='searchparam' value='' />"
        dataItem &= "<button class='button is-ghost' hx-post='searchcsvfile.cfm' hx-target='##viewcontainer'>SEARCH CSV</button>";
        dataItem &= "</form>";
        dataItem &= "</li>";
        dataitem &= "</ul>";
        writeOutput( dataItem );
    ...
    public function displayDataAsTable(required string fileName){
        ...
        orderedQuery = QueryExecute("select * from csvForOutput",{}, {dbtype="query"});
        ...
        for (headers in headerArray){
            tablevar &= "<th>" & headers & "</td>"; // dynamically build the header of the table as needed
        }
        tablevar &= "</tr></thead>"; // Close off the table header
        tablevar &= "<tbody>";
        for (datarows in orderedQuery){
            tablevar = tablevar & "<tr>";
            for (colname in headerArray){
                tablevar &= "<td>" & orderedQuery[colname][orderedQuery.currentRow] & "</td>";
            }
            tablevar &= "</tr>";
        }
    ...
    public function editCSV(required string fileName){
        ...
        previewcode = "<form>";
        previewcode &= "<input type='hidden' name='filename' value='" & theCSVFile & "'>";
        previewcode &= "<button class='button is-warning' id='btnsave' name='btnsave' hx-post='savecsvedits.cfm' hx-target='##viewcontainer'>SAVE CHANGES</button><div class='block'>&nbsp;</div>";
        previewcode &= "<textarea id='csvtext' name='csvtext' class='textareaedit'>" & csvObj & "</textarea>";
        previewcode &= "</form>";
    ...
    public function searchcsvfile(required string filename, required string searchparam){
        ...
        tablestring = '<table id="datatable" class="table table is-bordered table is-striped table is-fullwidth"><thead><tr>'; // start off the table
        for (headers in headerArray){
            tablestring &= "<th>" & headers & "</td>"; //dynamically build the header of the table as needed
        }
        tablestring &= "</tr></thead>"; // Close off the table header
        tablestring &= "<tbody>";
